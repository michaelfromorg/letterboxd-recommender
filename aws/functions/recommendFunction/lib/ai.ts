...
export async function getChatGPTRecommendations(
  ...
  const genrePrompt = genre
    ? `I'd like to watch a movie in the ${genre} genre. `
    : "";
  const alreadySeenPrompt =
    alreadySeen.length > 0
      ? `I've already seen ${alreadySeen
          .map((film) => film.name)
          .join(", ")} also. DO NOT recommend those.`
      : "";
...
The movie recommendations must be new movies, i.e., they should NOT be present within my reviews.${alreadySeenPrompt} Your response must be valid JSON.`;
  ...
  try {
    const recommendedFilms = JSON.parse(
      recommendationsJson
    ) as RecommendedFilm[];

    if (recommendedFilms.length !== 4) {
      console.warn(
        "Received unexpected number of recommendations:",
        recommendedFilms
      );
      return await getChatGPTRecommendations(
        seenFilms,
        reviewedFilms,
        alreadySeen,
        genre,
    ...
    if (newAlreadySeen.length >= 2) {
      console.warn(
        "Received more than 50% duplicate recommendations:",
        recommendedFilms
      );
      return await getChatGPTRecommendations(
        seenFilms,
        reviewedFilms,
        [...alreadySeen, ...newAlreadySeen],
        genre,
        retries - 1
      );
    }
