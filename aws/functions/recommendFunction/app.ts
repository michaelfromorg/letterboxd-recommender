...
export const lambdaHandler = async (
  ...
  console.log(
    `Received request for username: ${username}, genre: ${genre}, source: ${source}`
  );
  ...
  if (!source || !["Diary", "Reviews"].includes(source)) {
    return {
      ...
      statusCode: 400,
      body: JSON.stringify({
        message: "Missing source parameter or invalid source",
      }),
    };
  }
  ...
  try {
    console.log(
      "Fetching all user's seen films and reviewed films in parallel from Letterboxd"
    );

    const [seenFilmsResult, reviewedFilmsResult] = await Promise.all([
      fetchLetterboxdFilms(username),
      source === "Diary"
        ? fetchLetterboxdDiary(username)
        : fetchLetterboxdReviews(username),
    ]);
